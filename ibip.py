...
class ibipReport(UsesWhoisMixin, UsesBlocksMixin, BaseReport):
    page_name = "User:ST47/indef-blocked_ips"
    ...
    def format_actions(self, row):
        res  = "[https://en.wikipedia.org/wiki/Special:Block/"+row['user']+"?wpExpiry=1%20year 1 YEAR]"
        res += "[https://en.wikipedia.org/wiki/Special:Unblock/"+row['user']+" UNBLOCK]"
        return res
